...
if(count($rs2) > 0)
{
		...
		writeLog('mobile shifei money_now:' . $money_now);
}
else
{
  $rstr = "wrong";
...
if(abs($money_now) < 1e-6){
	...
	writeLog('mobile query sql:' . $query);
...
writeLog('mobile each data land_money_now:' . $land_money_now . '|land_money:' . $land_money . '|farmid:' . $farmid . '|shifeiren:' . $memberLogged_userName);
if(intval($farmid) < 11)
{
...
if(!isset($_COOKIE['shifei'])){
		...
		if(!empty($sHarvest)){
			...
			$db->query($query);
			writeLog('mobile h_harvest update:' . $query . '|shifeiren:' . $memberLogged_userName);
		}
		...
		writeLog("mobile shifei result:".$rstr."-".$huifei."-". $land_money_now ."-".$money_now . '|shifeiren:' . $memberLogged_userName);
