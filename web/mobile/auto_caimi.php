...
if($_GET['open']){
	...
	<script>
	alert('{$err}');
	window.location.href='com_list.php';
	</script>
	";
	}
}elseif($_GET['get']){
    //领取操作
	$sql2 = "select * from `h_member` where h_parentUserName = '{$memberLogged_userName}'";
	$us2 = $db->query($sql2);
	$fin_caimi = 0;
...
//自动采蜜 1级会员的
$query = "select * from  `h_usebee`  where  h_userName = '{$memberLogged_userName}'  and  h_fuserName = '{$username}' and h_level = 1  and  DATE_FORMAT(FROM_UNIXTIME(unix_timestamp(h_addTime)),'%Y-%m-%d') = '".$days."'";
...
if(!$result){
	if(!isset($_COOKIE['shouhuo'])){
		...
		if($result['num']){
				...
				if($money>0){
					...
					$result = $db->query($sql);
					if($result){
						getusebee($username,$memberLogged_userName,$money);
					}
    ...
    <div style="width: 100%;padding:10px 20px; ">
        <input type="button" onclick = "javascript:window.location.href=\'auto_caimi.php?open=1\'" value="确认提交"  class="friend-new-sub" />
    </div>
		';
		require_once 'f.php';
		require_once 'inc_footer.php';
	}else{
		redirect('auto_caimi.php?get=1');
	}
}
