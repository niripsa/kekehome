...
/*---------扩展或者增加金豆----------*/
	$(".di").click(function(){
		//if($(this).attr("id").replace('di','')>10) return false;
		//开垦新地播种事件	
		...
		$.post("addou.php",{addou:txt,kid:kaiID,adtype:adtypes,now:money_now},function(result){
			...
			if ( result.indexOf("ok")>0){
				if(adtypes=="ad"){
					...
					$("#di"+kaiID).html("<img src='"+imgsrc+"/shu"+getshu(result.split("-")[1],kaiID)+""+imgt+"' /><div class='diinfo'>"+result.split("-")[1]+"枚</div>");
					if(kaiID>10){	$("#di"+kaiID).css({background:"url("+imgsrc+"/dibg11"+imgt+") top left no-repeat"}); }
				...
				$("#body").css({cursor:""}); 
				if(adtypes=="ad"){
					zmei = $("#zmei").html().replace("枚","")*1)+txt;
				}
				else{
					zmei = $("#zmei").html().replace("枚","")+result.split("-")[1];
				}
				
				$("#zmei").html(zmei+"枚");
				$("#zhong").html($("#zhong").html().replace("枚","")-txt+"枚");
				/*alert(result);*/
				//location.reload();
				}
			else alert(result);
...
function getshu(dous,id){
	...
	{
		if (tdou==2) return 1;
		if (tdou>2 && tdou<=8) return 2;
		if (tdou>8 && tdou<=14) return 3;
		if (tdou>14 && tdou<=20) return 4;
	}
	else
	{
		if (tdou==2) return 5;
		if (tdou>2 && tdou<=8) return 6;
		if (tdou>8 && tdou<=14) return 7;
		if (tdou>14 && tdou<=20) return 8;
	}
