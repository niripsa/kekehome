...
//$act是从哪来的？	conn.php中foreach+$$_key
if($act == 'pa'){
	if(!$memberLogged){
		echo '您没有登录，请登录后再操作！';
		exit;
...
/*	$jd =dbmysql::getInstance($con_db_host,'shop','t2B2A4m8','shop');
	...
	if(!isset($_COOKIE['shoumi'])){
		...
		writeLog('id2_contains:' . json_encode($rs2));
		...
		echo '收米成功';
		writeLog('shoumi success:' . $num . ' time:' . date('Y-m-d H:i:s') . ' id:' . $id . '_str:' . $str);
		exit;
	}else{
...
/*	$jd =dbmysql::getInstance($con_db_host,'shop','t2B2A4m8','shop');
	...
	//}
	if(strlen($pwd) < 6 || strlen($pwd) > 32){
		echo '{"state":false,"msg":"登录密码6-32位任意字符，请检查！"}';//json
		exit;
	}
	
	$rs = $db->get_one("select * from `h_member` where h_userName = '{$username}'");
	//select 语句中出现{}是为了表明其中是一个单元
	//$a = "hello";
	//$b = "{$a}bcd";	hellobcd
	//echo $b;exit;
	if(!$rs){
