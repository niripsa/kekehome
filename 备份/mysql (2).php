...
class dbmysql {
	...
	function  __construct($con_db_host,$con_db_id,$con_db_pass, $con_db_name = '',$db_charset='utf8',$pconnect = 0) {
		//函数的默认参数，如果没有传入则取默认值
		if($pconnect) {
			if(!$this->link = mysqli_pconnect($con_db_host,$con_db_id,$con_db_pass,$con_db_name)) {
			...
			//mysql_connect()的第四个参数是
			//是否重复连接时使用新连接
			//若不填则仍为之前的连接，若填1则为新连接
		...
		if($this->version() > '4.1') {
			if($db_charset!='latin1') {
				mysql_query( $this->link,"SET character_set_connection=$db_charset, character_set_results=$db_charset, character_set_client=binary");
			}

			if($this->version() > '5.0.1') {
				mysql_query( $this->link,"SET sql_mode=''");
			}
		}
