...
/*
	...
	*/
	...
	if(isfull($memberLogged_userName)){
		...
		$should_num = $num + $num*0.3;
	}

	if($rs['h_point2'] < $should_num){
		echo '您的KK不足';
		exit;
	}
	/*
	...
	*/
	...
	//转账上限为总额的10%
	if($fAllMoney > 0){
		//$webInfo['h_sell_max'] = intval($fAllMoney * 0.1/10)*10;
	}
	
		$sell_time = date("m-d");
	if($rs['h_sell_day'] == $sell_time){
		if($rs['h_sell_num'] + $num > $webInfo['h_sell_max']){
		echo '每日最多提交转账'.$webInfo['h_sell_max'].'KK';
		exit;	
		}else{
			$n_sell_num = $rs['h_sell_num'] + $num;
			$n_sell_day = $sell_time;
		}
	}else{
		if($num > $webInfo['h_sell_max']){
		echo '每日最多提交转账'.$webInfo['h_sell_max'].'KK';
		exit;	
		}else{
		$n_sell_num = $num;
	...
	$sql = "update `h_member` set ";
	$sql .= "h_point2 = h_point2 - {$num}, ";
	$sql .= "h_sell_day = '" . $n_sell_day . "', ";
	$sql .= "h_sell_num = " . $n_sell_num . " ";
	$sql .= "where h_userName = '" . $memberLogged_userName . "' ";
	$db->query($sql);
	...
	if(!empty($memberLogged_userName) && !isfull($memberLogged_userName)){
		...
		//将转账手续费用记录到 系统收入表 h_log_point2_system
		$fWithDrawFee = $num * $toflower_rate*0.01;
		$sql = "insert into `h_log_point2_system` set ";
		$sql .= "h_userName = '" . $memberLogged_userName . "', ";
		$sql .= "h_price = {$fWithDrawFee}, ";
		$sql .= "h_about = '转账手续费归系统',"; 
		$sql .= "h_addTime = '" . date('Y-m-d H:i:s') . "', ";
		$sql .= "h_actIP = '" . getUserIP() . "', ";
		$sql .= "h_state = '转账成功待确认', ";
		$sql .= "h_remarks = '" . $h_remarks . "', ";
	...
	if($rs['h_passWordII'] != $pwd2){
		echo '安全密码错误';
		exit;
	}
	$endtime = strtotime('now')+3600;
	session_start();
	$_SESSION['pwd2']=$endtime;
	
	
	echo 'success';
}else if($act == 'shangcheng'){
	if(!$memberLogged){
		echo '您没有登录，请登录后再操作！';
		exit;
	}
...
/*    $jd =dbmysql::getInstance($con_db_host,'shop','t2B2A4m8','shop');
    $js2 = $jd->query("UPDATE ecs_users SET user_money = user_money + '{$num}' WHERE user_name = '{$memberLogged_userName}'");
    $jd->close();*/
	...
	session_start();
	if(!isset($_COOKIE['shoumi'])){
		setcookie('shoumi','shoumi', time()+20);
		//服务端加锁 事务
		$db->query_direct("START TRANSACTION");
		$rs =$db->get_one("select * from h_log_point2 where id={$id} for update");
		writeLog('mobile id_contains:' . json_encode($rs));

		if(empty($rs) || $rs['h_state'] == '1'){
			$str = "ROLLBACK1";
			$db->query_direct("ROLLBACK");
			exit;
		}
		...
		$sql .= "h_state = '1'";
		$sql .= " where id = {$id}";
		$bRet = $db->query_direct($sql);
		

		$h_remarks = $rs['h_remarks'];


		$rs2 =$db->get_one("select * from h_log_point2 where h_price>0 and h_remarks = '{$h_remarks}' for update");
		writeLog('mobile id_contains2:' . json_encode($rs2));

		if(empty($rs2) || $rs2['h_state'] == '1'){
			$str = "ROLLBACK2";
			$db->query_direct("ROLLBACK");
			exit;
		}
		...
		$sql2 .= "h_state = '1' ";
		$sql2 .= "where h_price>0 and h_remarks = '{$h_remarks}'";
		$bRet2 = $db->query_direct($sql2);

		//修改系统收入表 状态
		$sql3 = "update `h_log_point2_system` set ";
		$sql3 .= "h_state = '转账成功确认' ";
		$sql3 .= "where h_remarks = '{$h_remarks}'";
		$db->query_direct($sql3);
		

		//转入
		$rs =$db->get_one("select * from h_log_point2 where h_price>0 and h_remarks = '{$h_remarks}' for update");
		$h_member = $rs['h_userName'];
		$num = $rs['h_price'];

		$sql_select = "select * from `h_point2_lock` where lock_id = ${id} for update";
		$aLockInfo = $db->get_one($sql_select);
		if(!empty($aLockInfo)){
			$db->query_direct("COMMIT");
			writeLog('update error:' . json_encode($aLockInfo));
			exit;
		}
		...
		$bFlag = $db->query_direct($sql3);

		$insert_sql = "insert into `h_point2_lock` set lock_id=${id}, expire_time = '" . date('Y-m-d H:i:s') . "'";
		$bRet3 = $db->query_direct($insert_sql);

		if($bRet&&$bFlag&&$bRet2&&$bRet3){
			$str = "commit";
			$db->query_direct("COMMIT");
		}else{
			$str = "ROLLBACK";
		...
		echo '收米成功';
		writeLog('mobile shoumi success:' . $num . ' time:' . date('Y-m-d H:i:s') . ' id:' . $id . '_str:' . $str);
		exit;
	}else{
	...
	//手续费退回
		$sql = "insert into `h_log_point2` set ";
		$sql .= "h_userName = '" . $memberLogged_userName . "', ";
		$sql .= "h_price = '+" . ($num2 * $toflower_rate*0.01) . "', ";
		$sql .= "h_type = '退手续费', ";
		$sql .= "h_type_id = '9', ";
		$sql .= "h_about = 'KK转账取消，平台退回".$toflower_rate."%', ";
		$sql .= "h_addTime = '" . date('Y-m-d H:i:s') . "', ";
		$sql .= "h_actIP = '" . getUserIP() . "' ";
		$db->query($sql);

		//将扣除转账手续费用记录到 系统收入表 h_log_point2_system
		$fWithDrawFee = abs($num2 * $toflower_rate*0.01)*(-1);
		$sql = "insert into `h_log_point2_system` set ";
		$sql .= "h_userName = '" . $memberLogged_userName . "', ";
		$sql .= "h_price = {$fWithDrawFee}, ";
		$sql .= "h_about = '转账手续费从系统账户扣除',"; 
		$sql .= "h_addTime = '" . date('Y-m-d H:i:s') . "', ";
		$sql .= "h_actIP = '" . getUserIP() . "', ";
		$sql .= "h_state = '用户取消转账', ";
		$sql .= "h_type_id = 3";
	...
	$result = $db->query($query);
	$ci = 0;
	echo '<table border = "1" style="width:100%;text-align:center" class="table table-striped table-hover"><tr><td>单号ID</td><td>挂单金额</td><td>挂单时间</td><td>操作</td></tr>';
	while($rs_list = $db->fetch_array($result)){
	...
	if(!preg_match('#^\d{7,11}$#', $username)){
		echo '{"state":false,"msg":"用户名必须为正确手机号，请检查！"}';
		exit;
	}
	...
	//写入..
	$sql = "insert into `h_member` set ";
	$sql .= "h_parentUserName = '" . trim($comMember) . "', ";
	$sql .= "h_secondParentUserName = '" . strval($aParentInfo['h_parentUserName']) . "', ";
	$sql .= "h_userName = '" . trim($username) . "', ";
	...
	$jdpwd = $pwd;
	$jdusername = trim($username);
	//$jd =dbmysql::getInstance($con_db_host,'shop','t2B2A4m8','shop');
    //$js2 = $jd->query("INSERT INTO ecs_users (user_name,password,mobile_phone) VALUES ('{$jdusername}','{$jdpwd}','{$jdusername}')");
    //$jd->close();	
	...
	$rs = $db->get_one("select * from `h_member` where h_userName = '{$username}'");
	if(!$rs){
		echo '{"state":false,"msg":"账户不存在，请检查！"}';
		exit;
	}
	...
	if(strtolower(trim($verify)) != strtolower(trim($_SESSION['code']))){
		writeLog('time:' . date('Y-m-d H:i:s') . '_input verify code:' . $verify . ' produce code:' . $_SESSION['code']);
		echo '{"state":false,"msg":"验证码错误，请检查！"}';
		exit;
	}
